...
$(document).ready(function () {
    $('.salesPerson').submit(function (e) {

        e.preventDefault();
        var sales = $('salesPerson').val();
        $(".error").remove();


    if (sales(isLetter(value.charAt(0)) && isNumber(value.charAt(value.length - 1) == false))
    ) {
      ...
      this.volunteer = function() {
        ...
        // TODO 2(a) FR1.2
          $.post(
              ...
              {
                  OUCU: oucu, address: address, start_time: start, end_time: end, type:0

              },
              function (data) {
                  var obj = $.parseJSON(data);
                  if (obj.status == "success") {
                      alert("User " + oucu + " has been successfully registered a taxi offer.");
                  } else {
                      alert("User " + oucu + " has already made this offer.");
                  }
              }
          );
      };
      ...
      this.request = function() {
        ...
        // TODO 2(a) FR1.3
          $.post(
              "http://137.108.92.9/openstack/taxi/orders",
              {
                  OUCU: oucu, address: address, start_time: start,  type: 1

              },
              function (data) {
                  var obj = $.parseJSON(data);
                  if (obj.status == "success") {
                      alert("User " + oucu + " has successfully registered a request for a taxi share .");
                  } else {
                      alert("User " + oucu + " has already made this request.");
                  }
              }
          );
      };
...
// notify of new offers
      ...
      push.on('notification', (data) => {
          console.log("new offer or request! please check!");
      });
